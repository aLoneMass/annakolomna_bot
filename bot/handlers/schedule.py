...
async def send_schedule(message: Message): #Определяется асинхронная функция, которая принимает объект message.
    print (f'[DEBUG] Вызвана функция отправки расписания.')                                        #Это сообщение, к которому бот отвечает (например, при нажатии кнопки "📅 Расписание мероприятий").
    ...
    print(f'[DEBUG] список событий: {event}')
